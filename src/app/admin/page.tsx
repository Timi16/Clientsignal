    ...
    <AdminLayout title="Operations overview">
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 22 }} className="stat-grid">
        {([["Pending verifications", "2", "var(--amber)", "shield"], ["Leads in QA", "3", "var(--coral)", "flag"], ["Active attorneys", "2,412", "var(--verified)", "user"], ["Revenue (MTD)", "$486K", "var(--pine)", "dollar"]] as [string, string, string, string][]).map(([l, v, c, ic]) => (
          <div key={l} className="card" style={{ padding: 20 }}>
            <div className="row between" style={{ marginBottom: 12 }}><span className="eyebrow" style={{ fontSize: 10.5 }}>{l}</span><span style={{ color: c }}><Icon name={ic} size={18} /></span></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }} className="dash-grid">
        <div className="card" style={{ padding: 22 }}>
          <div className="row between" style={{ marginBottom: 16 }}><strong style={{ fontSize: 16 }}>Verification queue</strong><button onClick={() => router.push("/admin/verify")} className="row gap-1" style={{ fontSize: 13, color: "var(--pine)", fontWeight: 600 }}>Review all <Icon name="arrowR" size={15} /></button></div>
          <div className="stack" style={{ gap: 11 }}>
            {ATTORNEYS.filter(a => a.status !== "approved").map(a => (
              <div key={a.name} className="row between" style={{ padding: 13, borderRadius: 12, border: "1px solid var(--line)" }}>
        ...
        <div className="card" style={{ padding: 22 }}>
          <strong style={{ fontSize: 16, display: "block", marginBottom: 16 }}>Platform activity</strong>
