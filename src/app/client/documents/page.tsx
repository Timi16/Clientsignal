...
export default function ClientDocuments() {
  ...
  const { activeCase: c, loading } = useActiveCase();
  ...
  const onFile = (i: number) => {
    setItems(items.map((it, j) => j === i ? { ...it, status: "done", uploadedAt: new Date().toISOString() } : it));
  };
  const addItem = () => {
    if (!adding.trim()) return;
    setItems([...items, { id: crypto.randomUUID(), caseId: c.id, fileName: adding.trim(), status: "missing", required: false, uploadedAt: "" }]);
    setAdding("");
  };
    ...
    <ClientLayout title="Documents">
      <div style={{ maxWidth: 800 }}>
        ...
        <div className="row between" style={{ marginBottom: 18, flexWrap: "wrap", gap: 10 }}>
          <div className="row" style={{ gap: 10 }}><CaseTag type={c.practiceArea} /><span className="mono" style={{ fontSize: 12.5, color: "var(--text-3)" }}>{c.id}</span></div>
                ...
                <div className="row between" style={{ gap: 12, flexWrap: "wrap" }}>
                  <div className="row" style={{ gap: 13, flex: 1, minWidth: 0 }}>
                    ...
                    <div className="stack" style={{ gap: 4, minWidth: 0, flex: 1 }}>
                      <div className="row" style={{ gap: 8, minWidth: 0 }}><strong style={{ fontSize: 14.5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{it.fileName}</strong>{it.required && <span className="pill" style={{ background: "var(--coral-tint)", color: "var(--coral)", fontSize: 10, padding: "2px 7px", flex: "none" }}>Important</span>}</div>
                      <span className="row" style={{ gap: 6, fontSize: 12.5, color: s[1], minWidth: 0 }}><Icon name={s[3]} size={13} /> <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{it.uploadedAt || s[0]}</span></span>
                    </div>
                  </div>
                  {it.status === "done" ? (
