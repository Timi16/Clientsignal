...
export default function ClientTimeline() {
  ...
  const { cases, activeCase, attorney, setActiveId, loading } = useActiveCase();
  const [sel, setSel] = useState(activeCase?.id || "");
  const c = cases.find(x => x.id === sel) || cases[0] || null;
  const attyName = c && sel === activeCase?.id && attorney ? attorney.name : null;
  const steps = c ? buildTimeline(c.stage, attyName) : [];
    ...
    <ClientLayout title="Case Timeline">
      <div style={{ maxWidth: 900 }}>
        ...
        <div className="row" style={{ gap: 10, marginBottom: 22, flexWrap: "wrap" }}>
          {cases.map(x => {
            const st = CASE_STATUS[x.status] || CASE_STATUS.pending;
            const on = x.id === sel;
            return (
              <button key={x.id} onClick={() => { setSel(x.id); setActiveId(x.id); }} className="card" style={{ padding: "12px 16px", textAlign: "left", border: "1.5px solid " + (on ? "var(--signal)" : "var(--line)"), background: on ? "var(--blue-tint)" : "var(--card)", display: "flex", alignItems: "center", gap: 11 }}>
                <span style={{ width: 32, height: 32, borderRadius: 9, background: CASE_TYPES[x.practiceArea]?.tint, display: "grid", placeItems: "center", flex: "none" }}><span style={{ width: 11, height: 11, borderRadius: "50%", background: CASE_TYPES[x.practiceArea]?.color }} /></span>
                <div className="stack" style={{ gap: 2 }}>
                  <strong style={{ fontSize: 13.5, whiteSpace: "nowrap" }}>{CASE_TYPES[x.practiceArea]?.label}</strong>
        ...
        <div className="card" style={{ padding: 28 }}>
          <div className="row between" style={{ marginBottom: 24, flexWrap: "wrap", gap: 10 }}>
            <div className="row" style={{ gap: 11 }}><CaseTag type={c.practiceArea} /><span className="mono" style={{ fontSize: 12.5, color: "var(--text-3)" }}>{c.id}</span></div>
            ...
            {steps.map((e, i) => (
              <div key={i} className="row" style={{ gap: 16, alignItems: "flex-start" }}>
                ...
                <div className="stack" style={{ gap: 3, paddingBottom: 26, flex: 1 }}>
                  ...
                  {e.current && attyName && <button className="btn btn-signal btn-sm" style={{ alignSelf: "flex-start", marginTop: 8 }} onClick={() => router.push("/client/messages")}>Reply to {attyName.split(" ")[0]} <Icon name="arrowR" size={15} /></button>}
                  {e.current && !attyName && <span className="row gap-1" style={{ fontSize: 13, color: "var(--amber)", fontWeight: 600, marginTop: 6 }}><span className="pulse-dot" style={{ background: "var(--amber)" }} /> Matching in progress…</span>}
                </div>
              </div>
            ))}
          </div>
