...
export default function ClientDashboard() {
  ...
  const { activeCase: c, cases, attorney: atty, loading } = useActiveCase();
  const { user } = useAuth();
  ...
  if (c.unreadCount > 0) nextSteps.push({ t: `Reply to ${atty ? atty.name.split(" ")[0] + "'s" : "your attorney's"} message`, d: "They're waiting to hear back from you", icon: "message", cta: "Reply", dest: "/client/messages", hot: true });
  docsNeeded.filter(d => d.status === "requested").forEach(d => nextSteps.push({ t: `Upload: ${d.fileName}`, d: "Requested by your attorney", icon: "upload", cta: "Upload", dest: "/client/documents", hot: !c.unreadCount }));
    ...
    <ClientLayout title="My Case">
      <div className="row between" style={{ gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
        <div className="stack" style={{ gap: 4 }}>
          <h2 className="display" style={{ fontSize: 30 }}>Hi {firstName} — {c.status === "pending" ? "we're on it." : c.status === "closed" ? "welcome back." : "good news."}</h2>
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => router.push("/client/cases")}><Icon name="inbox" size={15} /> All cases ({cases.length})</button>
      </div>
      ...
      <div className="card" style={{ padding: 26, marginBottom: 18 }}>
        <div className="row between" style={{ marginBottom: 22, flexWrap: "wrap", gap: 10 }}>
          <div className="row" style={{ gap: 10 }}>
            ...
            <CaseTag type={c.practiceArea} sm />
          </div>
          <span className="mono" style={{ fontSize: 12.5, color: "var(--text-3)" }}>Case {c.id} · opened {opened}</span>
        </div>
          ...
          {atty ? (
            <div className="card" style={{ padding: 30, minHeight: 226, background: "var(--pine)", color: "#fff" }}>
              ...
              <div className="row" style={{ gap: 17, marginTop: 18 }}>
                ...
                <div className="stack" style={{ gap: 4 }}>
                  ...
                  <span style={{ fontSize: 13.5, color: "rgba(234,240,249,0.72)" }}>{atty.firmName} · {atty.yearsExperience} yrs</span>
            ...
            <div className="card" style={{ padding: 30, minHeight: 226, background: "var(--pine)", color: "#fff", textAlign: "center" }}>
              <div className="stack" style={{ alignItems: "center", gap: 12 }}>
                ...
                <p style={{ fontSize: 13.5, color: "rgba(234,240,249,0.7)", lineHeight: 1.5, maxWidth: 320 }}>We&apos;re matching your {caseType.label.toLowerCase()} case with a verified attorney in {c.city}. You&apos;ll be notified the moment we do.</p>
        ...
        <div className="stack" style={{ gap: 22 }}>
          <div className="card" style={{ padding: 26, minHeight: 420 }}>
            ...
            <div className="stack" style={{ gap: 15 }}>
              {([["Type", caseType.label], ["Matter", c.matter], ["Location", c.city], ["Opened", opened]] as [string, string][]).map(([k, v]) => (
            ...
            <div className="row" style={{ gap: 14, marginTop: 22, padding: 18, borderRadius: 12, background: c.strengthScore >= 80 ? "var(--verified-tint)" : "var(--amber-tint)" }}>
              <ScoreRing value={c.strengthScore} size={48} stroke={5} color={c.strengthScore >= 80 ? "var(--verified)" : "var(--amber)"} />
              <div className="stack" style={{ gap: 2 }}><strong style={{ fontSize: 14, color: c.strengthScore >= 80 ? "var(--verified)" : "var(--amber)" }}>{c.strengthScore >= 80 ? "Strong case" : "Good case"}</strong><span style={{ fontSize: 12.5, color: "var(--text-2)" }}>Your details &amp; documents shape your case strength.</span></div>
            </div>
          </div>
